...
public class TestCaptainJobletWithRammingSpeed {
  ...
  public void setup() {
    ...
    manifestFactoryMap.put(
        CaptainAppType.fromString("APP_TYPE_1"), new TestManifestFactory(testManifest));
    manifestFactoryMap.put(
        CaptainAppType.fromString("APP_TYPE_2"), new TestManifestFactory(testManifest2));
  ...
  public void testGoToNextStep() throws Exception {
    ...
    inOrder
        .verify(requestUpdater, atLeastOnce())
        .setStepAndStatus(
            config.getId(),
            CAPTAIN_STEP1,
  ...
  public void testGoToNextStepInitializing() throws Exception {
    ...
    new CaptainJoblet(
            config, notifier, requestUpdater, manifestManager, true, true, FAILED_REQUEST_POLICY)
        .run();
