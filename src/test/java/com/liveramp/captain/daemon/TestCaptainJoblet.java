...
public class TestCaptainJoblet {
  ...
  @Mock private CaptainNotifier notifier;

  @Mock private RequestSubmitter<Long> requestSubmitterStep1;
  @Mock private RequestSubmitter<Long> requestSubmitterStep2;
  @Mock private RequestSubmitter<Long> requestSubmitterStep4;
  @Mock private RequestSubmitter<Map<Long, Long>> requestSubmitterStep5;
  @Mock private RequestSubmitter<Long> requestSubmitterStep6;

  @Mock private HandlePersistor<Long> handlePersistor2;
  @Mock private HandlePersistor<Long> handlePersistor4;
  @Mock private HandlePersistor<Map<Long, Long>> handlePersistor5;

  @Mock private StatusRetriever statusRetrieverStep2;
  @Mock private StatusRetriever statusRetrieverStep3;
  @Mock private StatusRetriever statusRetrieverStep4;
  @Mock private StatusRetriever statusRetrieverStep5;
  ...
  @Mock private FailedRequestPolicy failedRequestPolicyMock;
    ...
    Manifest testManifest =
        new DefaultManifestImpl(
            Lists.newArrayList(
                ...
                new AsyncWaypoint(
                    CAPTAIN_STEP2_B, requestSubmitterStep2, handlePersistor2, statusRetrieverStep2),
                new ControlFlowWaypoint(CAPTAIN_STEP3, statusRetrieverStep3),
                new AsyncWaypoint(
                    CAPTAIN_STEP4, requestSubmitterStep4, handlePersistor4, statusRetrieverStep4),
                new AsyncWaypoint(
                    CAPTAIN_STEP5, requestSubmitterStep5, handlePersistor5, statusRetrieverStep5),
  ...
  public void testGoToNextStep() throws Exception {
    ...
    final CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.COMPLETED, CAPTAIN_STEP1, APP_TYPE_1);

    new CaptainJoblet(
            config, notifier, requestUpdater, manifestManager, true, false, FAILED_REQUEST_POLICY)
        .run();
  ...
  public void testGoToNextStepInitializing() throws Exception {
    final CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.COMPLETED, INITIALIZING, APP_TYPE_1);
  ...
  public void testGoToNextStepLastStep() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.COMPLETED, CAPTAIN_STEP6, APP_TYPE_1);
    ...
    verify(requestUpdater, times(1))
        .setStepAndStatus(
            config.getId(), CAPTAIN_STEP6, CaptainStatus.COMPLETED, DONE, CaptainStatus.COMPLETED);
  }

  @Test
  public void testSubmitRequestAsync() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.READY, CAPTAIN_STEP2_B, APP_TYPE_1);

    when(requestSubmitterStep2.submit(eq(JOB_ID), any(RequestContext.class)))
        .thenReturn(SERVICE_HANDLE);
  ...
  public void testSubmitRequestAsyncSupportsPendingFalse() throws Exception {
    ...
    new CaptainJoblet(
            config, notifier, requestUpdater, manifestManager, false, false, FAILED_REQUEST_POLICY)
        .run();
  ...
  public void testSubmitRequestSync() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.READY, CAPTAIN_STEP1, APP_TYPE_1);
  ...
  public void testSubmitRequestSyncWithHandlerPersistor() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.READY, CAPTAIN_STEP2_A, APP_TYPE_1);
  ...
  public void testSubmitRequestFlowControl1() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.READY, CAPTAIN_STEP3, APP_TYPE_1);
    ...
    verify(requestUpdater, times(1))
        .setStatus(config.getId(), CAPTAIN_STEP3, CaptainStatus.READY, CaptainStatus.PENDING);
  }

  @Test
  public void testCheckRequestStartedInProgress() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.PENDING, CAPTAIN_STEP2_B, APP_TYPE_1);
    ...
    verify(requestUpdater, times(3))
        .setStatus(
            config.getId(), CAPTAIN_STEP2_B, CaptainStatus.PENDING, CaptainStatus.IN_PROGRESS);
  }
  ...
  public void testCheckRequestStartedQuarantine() throws Exception {
    ...
    verify(requestUpdater, times(2))
        .setStatus(
            config.getId(), CAPTAIN_STEP2_B, CaptainStatus.PENDING, CaptainStatus.QUARANTINED);
  }
  ...
  public void testRequestCompleteCompleted() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.IN_PROGRESS, CAPTAIN_STEP2_B, APP_TYPE_1);
    ...
    verify(requestUpdater, times(1))
        .setStatus(
            config.getId(), CAPTAIN_STEP2_B, CaptainStatus.IN_PROGRESS, CaptainStatus.COMPLETED);
  }
  ...
  public void testRequestCompleteQuarantined() throws Exception {
    ...
    verify(requestUpdater, times(2))
        .setStatus(
            config.getId(), CAPTAIN_STEP2_B, CaptainStatus.IN_PROGRESS, CaptainStatus.QUARANTINED);
  }
  ...
  public void testRequestCompleteFailed() throws Exception {
    ...
    verify(requestUpdater, times(1))
        .setStatus(
            config.getId(), CAPTAIN_STEP2_B, CaptainStatus.IN_PROGRESS, CaptainStatus.FAILED);
    verify(requestUpdater, times(0))
        .setStatus(
            config.getId(), CAPTAIN_STEP2_B, CaptainStatus.IN_PROGRESS, CaptainStatus.QUARANTINED);
  }
  ...
  public void testRequestCompleteCompletedFlowControl() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.IN_PROGRESS, CAPTAIN_STEP3, APP_TYPE_1);
    ...
    verify(requestUpdater, times(1))
        .setStatus(
            config.getId(), CAPTAIN_STEP3, CaptainStatus.IN_PROGRESS, CaptainStatus.COMPLETED);
  }

  @Test
  public void testUsesCorrectManifestForAppType() throws Exception {
    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.COMPLETED, CAPTAIN_STEP1, APP_TYPE_2);
  ...
  private void testFailedRequestPolicy(
      ...
      VerificationMode timesQuarantine)
      throws Exception {

    CaptainRequestConfig config =
        new SimpleCaptainConfig(JOB_ID, CaptainStatus.FAILED, CAPTAIN_STEP1, APP_TYPE_2);

    when(failedRequestPolicyMock.getFailedRequestAction(JOB_ID)).thenReturn(mockedAction);
    new CaptainJoblet(
            config, notifier, requestUpdater, manifestManager, true, false, failedRequestPolicyMock)
        .run();
